...
if __name__ == "__main__":
    ...
    data = pd.read_csv(
        r"~/replication_study/model_fitting/{0}.csv".format(dat)
    )    
    
    data = data[data["krakenPresent"] == kraken_present]
    ...
    grids = pd.read_json("~/replication_study/model_fitting/sample_grid.json")
    ...
    estimates = pd.read_csv(r"~/replication_study/model_fitting/estimates{0}.csv".format(decision_model))
    ...
    if decision_model == "CB" or decision_model == "CB_n":
        ...
        beta_subject = np.repeat(0, N_SUBS_SIM)
    ...
    if decision_model == "CB" or decision_model == "CB_b0":
...
df.to_csv('~/replication_study/model_fitting/results/parameter_recovery/{0}/simdat_kraken_{1}.csv'.format(decision_model.lower(), kraken_present))
