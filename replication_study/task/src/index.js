...
var subjectID 
if (!demo){
  subjectID = jatos.studySessionData.valueOf()["ID"];
}




// if subjectID is undefined, make a random one
if (subjectID == undefined) {
  
  subjectID =  'test-' +Math.floor(Math.random() * 1000000000);
}
...
var uid
if (!demo){
  uid = jatos.studySessionData.valueOf()["uid"];
}
...
blocks.splice(4,0,0) // insert a no-kraken round in the 6th round for the bonus round
blocks.splice(0,0,0) // insert a no-kraken round in the 1st round for the training
    ...
    if (currentBlock -1 < blocks.length) { //KW: if still has blocks to go, empty trial data arrays and run the task again (-2 bc 1 block for intro and 1 for bonus)
      if (understood){
       ...
       // disp comprehension questions
        startComprehension()
        page7[comprehensionAttempts -1] = (Number(new Date()) - time6)/60000
...
function checkComprehension(){
  ...
  var i
    ...
    if (incomplete.length > 0) {
      ...
      $(document.getElementById("mainSection")).show()
      ...
      var exp = [];
          if (values["Q1_0"] != "2") {
              incorrect.push("What is the aim of this game?")
              exp.push("The aim of this game is to <u>catch as many fish as possible</u>. The kraken is something you want to avoid.")
          }
          if (values["Q2_1"] != "0") {
              incorrect.push("What happens when you click the same square several times?")
              exp.push("When you click the same square several times, you will get <u>approximately the same number of fish</u> (+-2).<br>"+
                "The number of fish <u>will not</u> decrease when you click the same square several times.")
          }
          if (values["Q3_2"] != "2") {
              incorrect.push("How do you know how many fish to expect in one location?")
              exp.push("The number of fish in adjacent squares is <u>similar</u>. The closer two squares are, the more similar the number of fish in them will be.")
          }
          if (values["Q4_3"] != "1") {
              incorrect.push("What happens when you find the kraken?")
              exp.push("When you find the kraken, the round is over and you <u>lose all the fish you collected in that round</u>. You must try to avoid this.")
          }
          if (values["Q5_4"] != "2") {
              incorrect.push("When can you expect to find the kraken?")
              exp.push("You can expect to find the kraken when you click a square with <u>less than 50 fish</u> but only on a round where it is written that the <u>kraken is nearby</u> over the ocean.")
          }
          if (values["Q6_5"] != "1") {
              incorrect.push("Does the number of fish in each square change from one block to the next?")
              exp.push("Yes, each block is a <u>completely new ocean</u>. The number of fish in each square is different in each block.")
          }
      ...
      $(instructionHeading).hide()

      window.scrollTo(0,0);

     // explain what they did wrong
     document.getElementById('instructions').innerHTML = "<h1>You answered one or more questions incorrectly.</h1>"
 
     document.getElementById('instructions').innerHTML += "<h1>Here are the question(s) that you answered incorrectly: <br> <br></h1> "
         var i
         for (i = 0; i < incorrect.length; i++) {
             document.getElementById('instructions').innerHTML +="<h2 style ='color:Tomato'><b>" + incorrect[i] + "</b></h2>"
              document.getElementById('instructions').innerHTML +="<h2>" + exp[i] + "</h2><br><br>"

         }
      document.getElementById('instructions').innerHTML += "<h2>Click the button below to return to the comprehension questions.</h2> "
      
      // make button to go back to the comprehension questions
      
     document.getElementById("comprehension").style.display = 'block';

     document.getElementById("comprehension").onclick = function(){
      $(document.getElementById("comprehension")).hide()
      window.scrollTo(0,0);
      document.getElementById("instructions").innerHTML = "";
      $(document.getElementById("quizContainer")).show()
      $(document.getElementById("quiz")).show()
...
var init = function() {
  ...
  instructionsArray.push(
    ...
    "When the kraken is nearby, there will be a warning above the ocean saying 'the kraken is nearby'. Sometimes the kraken will be somewhere else, and you won't be at risk of finding it. In that case, the text above the grid will say 'the kraken is feeding elsewhere'.");
    ...
    "<br><br>Specifically, find 50 fish or less, that means you have disturbed the Kraken, and will lose your catch! <br>" +
    "<br><br>On the other hand, if you fish in location and find <b>more than 50 fish</b>, you know that area is safe. Importantly, the ocean is deep, and the <b>number of fish</b> you collect <b>will not go down</b> if you decide to keep fishing in the same location.<br><br>" + 
  ...
  instructionsArray.push("Some people have told us that the game made them feel nervous.<br>We will therefore ask you from time to time <u>how nervous</u> you felt during the previous round.<br>Please answer this <u>honestly</u> and not based on what you think we would like to hear!<br><br>" +
  ...
  arrow.addEventListener("mouseup", function() {
    ...
    // info on nervousness
    if (training_iteration == 6) {
      time7 = Number(new Date())
      page6[comprehensionAttempts - 1] = (time7 - time6)/60000;
      training_clickable = true
      instructionText.classList.add("fade");
      instructionText.innerHTML = instructionsArray[6];
...
// KW: create the function that lets you create an SVG w/o always entering that url (used to create basics of the grid)
  ...
  jQuery.getJSON('assets/sample_grid.json').done(function(data) {
    grids = data;
  }).then(function() {
    if (currentBlock == 0) {
