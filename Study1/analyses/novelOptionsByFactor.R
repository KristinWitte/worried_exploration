...
load("~/safe_exploration/master.Rda")
load("~/safe_exploration/factorScores.Rda")
...
Master$anx <- factorScores$AnxDepr[match(Master$ID, factorScores$ID)]
Master$ext <- factorScores$ext[match(Master$ID, factorScores$ID)]
Master$neuro <- factorScores$neuroDev[match(Master$ID, factorScores$ID)]
Master$withdraw <- factorScores$withdraw[match(Master$ID, factorScores$ID)]

# z scale all numeric predictors first

Master[,c(2,3,6,11:20,22:24, 26:29)] <- sapply(Master[,c(2,3,6,11:20,22:24, 26:29)], function(df) scale(df, center = T, scale = T))
head(Master)
...
if (!file.exists("~/safe_exploration/NUOz_factor")){
  
  dir.create(file.path("~/safe_exploration/NUOz_factor"))}
...
formula <- as.formula(paste("uniqueFROMz~", questionnaires[task_id], "*krakenPres + z *", questionnaires[task_id], " + click + blocknr + age + gender + edu + (click + blocknr + krakenPres + z| ID)"))
...
save.image(paste("~/safe_exploration/NUOz_factor/",questionnaires[task_id],".Rdata", sep = ""))
