## this script only fits the models to the different cross-validation folds
# the likelihood is computed in the run_cross_validation_likelihood.py script
import os
try:
   ...
   # data.loc[data["block"] > 5, "block"] -= 1
    ...
    print(
        "Saving to NetCDF, location = {0}".format(
            os.path.join(
                output_dir,
                "model_fit__cond-{0}_tp-{3}_fold-{1}_run-{2}.nc".format(
                    cond, cv_fold, runID, tp
                ),
            )
        )
    )
